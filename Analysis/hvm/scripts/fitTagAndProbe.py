...
elif args.sig_model == 'CBShape':
    pdf_args.extend(
         [
            "CBShape::signalPass(m_ll, mean[3.1,3.0,3.2], sigma[0.05,0.02,0.06], alpha[3., 0.5, 5.], n[1, 0.1, 100.])",
            "CBShape::signalFail(m_ll, mean[3.1,3.0,3.2], sigma[0.05,0.02,0.06], alpha[3., 0.5, 5.], n[1, 0.1, 100.])",
         ]
    )
...
if args.bkg_model == 'Exponential':
    pdf_args.extend(
            [
                ...
                #"Exponential::backgroundPass(m_ll, lp[-0.1,-5,5])",
                #"Exponential::backgroundFail(m_ll, lf[-0.1,-5,5])"
...
elif args.bkg_model == 'Hists':
    pass
elif args.bkg_model == 'HistPass':
    pdf_args.extend(
            [
                "Exponential::backgroundFail(m_ll, lf[-0.1,-1,0.1])"
            ]
...
for b in bins:
    ...
    plot.Set(axis.GetXaxis().SetTitle('m_{tag-probe} (GeV)'))
    plot.Set(axis.GetYaxis().SetTitle('Events / %g GeV' % width))
    plot.DrawTitle(pads[1], '35.9 fb^{-1} (13 TeV)', 3)
