...
def Chi2Compatibility(*measurements):
    ...
    print pval
    return pval
# def MakeRatio(num, den, rebin=None):
#     h_fr_mc = num.Clone()
#     h_den = den.Clone()
...
pt_ranges = ['30_40', '40_60', '60_100']

results = []

for region in ['barrel1', 'barrel2', 'endcap1', 'endcap2']:
    for pt_range in pt_ranges:
        post = '_pt_%s' % pt_range
        for sel in selections:
            for i, chn in enumerate(channels):
                seldir = '%s_%s_%s%s' % (region, chn, sel, post)
                seldir_cmb = '%s_%s%s' % (region, sel, post)
                node = hists[chn][seldir][var]
                node['data_sub'] = (node['data_obs'] - ((node['Total_R'] * 1.0) + node['Total_E']))

                for path, key, val in hists[chn][seldir][var].ListObjects():
                    if i == 0:
                        hists['cmb'][seldir_cmb][var][key] = val.Clone()
                    else:
                        hists['cmb'][seldir_cmb][var][key].Add(val)
        ...
        proc = 'Total_J'
        ratio = plot.MakeRatioHist(hists['cmb'][seldir_num][var][proc], hists['cmb'][seldir_den][var][proc], True, True)


        # fakefrac_num = hists['cmb'][seldir_num][var][proc].Clone()
        ...
        # print '>> True ratio = %f' % true_ratio
        true_ratio = IntegralRatio(hists['cmb'][true_num][var][proc], hists['cmb'][true_den][var][proc])
        ratio.Fit('pol2', '', '', 3, 20)

        mean = hists['cmb'][true_den][var][proc].GetMean()
        # print 'Mean = %f' % mean

        func = ratio.GetListOfFunctions().At(0)
        fitval = func.Eval(mean)
        ...
        fiterr = hint.GetBinError(hint.GetXaxis().FindFixBin(mean))
        fit_ratio = (fitval, fiterr)

        resline = '%-20s %-20s | %f +/- %f | %f +/- %f' % (region, pt_range, true_ratio[0], true_ratio[1], fit_ratio[0], fit_ratio[1])
        acc = (fitval - true_ratio[0]) / fiterr
        resline += ' | %f' % acc

        resline += ' | %f' % Chi2Compatibility(fit_ratio, true_ratio)
        results.append(resline)
        # results.append('%-20s %-20s | %f +/- %f | %f +/- %f' % (region, pt_range, true_ratio[0], true_ratio[1], fitval, fiterr))
        # hint.Print('range')
        # h_fr.GetFunction('pol2').Draw('E3SAME')

        canv = ROOT.TCanvas('photon_fakes_%s_%s%s' % (args.year, region, post), 'photon_fakes_%s_%s%s' % (args.year, region, post))
        pads = plot.OnePad()
