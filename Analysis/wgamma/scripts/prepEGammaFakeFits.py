...
if args.fine:
    eta_bins = [-2.5, -1.4442, -1.0, -0.5, 0, 0.5, 1.0, 1.4442, 2.5]
else:
...
hist.GetYaxis().SetTitle(varname)
    ...
    for ieta in range(1, hist.GetNbinsY() + 1):
        for ipt in range(1, hist.GetNbinsX() + 1):
            binname = '%s>=%g && %s<%g && %s>=%g && %s<%g' % (
                    ...
                    varname, hist.GetYaxis().GetBinLowEdge(ieta),
                    varname, hist.GetYaxis().GetBinUpEdge(ieta),
                    )
