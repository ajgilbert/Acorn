...
if args.special == 'eft_region':
    plot.ModTDRStyle(width=750, height=670, l = 0.19, b = 0.19, r = 0.23, t=0.06)
else:
...
if args.special == 'eft_region':
    ...
    n_bins_y = 3
    for ix in xrange(10):
      hist.GetXaxis().SetBinLabel(ix * n_bins_y + 1, '[0, #pi/6]')
      hist.GetXaxis().SetBinLabel(ix * n_bins_y + 2, '[#pi/6, #pi/3]')
      hist.GetXaxis().SetBinLabel(ix * n_bins_y + 3, '[#pi/3, #pi/2]')
      hist.GetYaxis().SetBinLabel(1 + hist.GetNbinsY() - (ix * n_bins_y + 1), '[0, #pi/6]')
      hist.GetYaxis().SetBinLabel(1 + hist.GetNbinsY() - (ix * n_bins_y + 2), '[#pi/6, #pi/3]')
      hist.GetYaxis().SetBinLabel(1 + hist.GetNbinsY() - (ix * n_bins_y + 3), '[#pi/3, #pi/2]')
      latex = ROOT.TLatex()
      x_offset = -4.3
      plot.Set(latex, TextSize=0.017, TextAlign=22)
      latex.DrawLatex(1.5, x_offset, '[150, 200]')
      latex.DrawLatex(4.5, x_offset, '[200, 300]')
      latex.DrawLatex(7.5, x_offset, '[300, 500]')
      latex.DrawLatex(10.5, x_offset, '[500, 800]')
      latex.DrawLatex(13.5, x_offset, '[800, 1200]')
      latex.DrawLatex(15 + 1.5, x_offset, '[150, 200]')
      latex.DrawLatex(15 + 4.5, x_offset, '[200, 300]')
      latex.DrawLatex(15 + 7.5, x_offset, '[300, 500]')
      latex.DrawLatex(15 + 10.5, x_offset, '[500, 800]')
      latex.DrawLatex(15 + 13.5, x_offset, '[800, 1200]')

      plot.Set(latex, TextSize=0.04, TextAlign=22)
      latex.DrawLatex(7.5, x_offset - 1.4, 'l^{-}')
      latex.DrawLatex(15 + 7.5, x_offset - 1.4, 'l^{+}')

      # latex.DrawLatex(16, x_offset, '[850, 1200]')

      plot.Set(latex, TextAngle=90, TextSize=0.017)

      y_offset = -4.4
      latex.DrawLatex(y_offset, 1.5,  '[800, 1200]')
      latex.DrawLatex(y_offset, 4.5,  '[500, 800]')
      latex.DrawLatex(y_offset, 7.5,  '[300, 500]')
      latex.DrawLatex(y_offset, 10.5, '[200, 300]')
      latex.DrawLatex(y_offset, 13.5, '[150, 200]')
      latex.DrawLatex(y_offset, 15 + 1.5,  '[800, 1200]')
      latex.DrawLatex(y_offset, 15 + 4.5,  '[500, 800]')
      latex.DrawLatex(y_offset, 15 + 7.5,  '[300, 500]')
      latex.DrawLatex(y_offset, 15 + 10.5, '[200, 300]')
      latex.DrawLatex(y_offset, 15 + 13.5, '[150, 200]')
      plot.Set(latex, TextSize=0.04)
      latex.DrawLatex(y_offset - 1.4, 7.5, 'l^{+}')
      latex.DrawLatex(y_offset - 1.4, 15 + 7.5, 'l^{-}')
