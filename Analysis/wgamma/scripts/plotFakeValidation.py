...
f_old = ROOT.TFile('output_total_baseline_200623-mc-photon-fakes.root')
f_new = ROOT.TFile('output_total_baseline_200623-mc-true-photon-fakes.root')
...
for h in [
    ...
    ('l0p0_dr', '#DeltaR(#mu,#gamma)'),
    ('p0_eta', 'Photon #eta')]:
    hists = {
        'Total_J': f_old.Get('m/baseline_m_mZ_veto/%s/Total_J' % h[0]),
        'Total_J_fw': f_old.Get('m/baseline_m_mZ_veto/%s/Total_J_fw' % h[0]),
        'Total_J_fwt': f_new.Get('m/baseline_m_mZ_veto/%s/Total_J_fw' % h[0])
    }
    ...
    MakeMultiHistPlot('photon_fake_validation_%s' % h[0],
                      outdir='.',
                      hists=hists,
                      ...
                      layout=[
                          {'name': 'Total_J', 'legend': 'Nominal'},
                          {'name': 'Total_J_fw', 'legend': '#sigma_{i#etai#eta} extrap.'},
                          {'name': 'Total_J_fwt', 'legend': '#sigma_{i#etai#eta} extrap. true'},
                      ],
                      ratios=[
                          {'num': 'Total_J_fw', 'den': 'Total_J'},
                          {'num': 'Total_J_fwt', 'den': 'Total_J'},
                      ]
    )
