...
def DoFilters(path, menu, run, save, append=''):
    ...
    out = subprocess.check_output(['hltConfigFromDB', '--runNumber', '%s' % run, '--cff', '--paths', path])
...
for i, x in enumerate(trg_info):
    ...
    if curr_v is None:
        ...
        mod = DoFilters(path, menu, run, args.save_cfg)
    ...
    if curr_v is not None and v == curr_v and curr_L1_set != L1_set:
        ...
        print '    * L1T seed: %s' % (L1)
        print ' - V%i filter sequence:' % (v)
        mod = DoFilters(path, menu, run, args.save_cfg, '_altL1_' + str(L1_alt_idx))
        L1_alt_idx += 1
